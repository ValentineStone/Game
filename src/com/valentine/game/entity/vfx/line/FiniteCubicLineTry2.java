...
public class FiniteCubicLineTry2 extends Line
{
	...
	private void refillMatrix()
	{
		...
		/*
		 *
		 * A[0][0] = 3*dots.get(0).x*dots.get(0).x; A[0][1] = 2*dots.get(0).x;
		 * A[0][2] = 1;
		 *
		 * int k = 4*(dots.size())-1; int p = 4*(dots.size()-1);
		 *
		 */

		/*
		 * A[k][p+0] = 6*dots.get(0).x; A[k][p+1] = 2; b[k] = 0;
		 */
		...
		/*
		 *
		 * A[0][0] += 6*dots.get(0).x; A[0][1] += 2; //System.err.println("A[" +
		 * (dots.size()-1) + "][" + (4*(dots.size())-1) + "]");
